...
#define OBJ_PS_TABLE 					19474
#define GUI_PS_TABLE						0
#define MAX_POKERTABLES 					100
#define MAX_POKERTABLEMISCOBJS				6
#define MAX_PLAYERPOKERUI					48
#define DRAWDISTANCE_PS_TABLE 			150.0
#define DRAWDISTANCE_PS_MISC 			50.0
#define CAMERA_PS_INTERPOLATE_SPEED		5000 // ms (longer = slower)
#define DIALOG_BUYPOKER 					10042
...
{
    InitPokerTables();
    #if defined PS_OnGameModeInit
		return PS_OnGameModeInit();
	#else
		return 1;
	#endif
}
...
#endif
#define OnGameModeInit PS_OnGameModeInit
#if defined PS_OnGameModeInit
	forward PS_OnGameModeInit();
#endif

public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    // Poker
	if(dialogid == DIALOG_BUYPOKER && response)
	{
		switch (listitem)
		{
			case 0:
			{
			    if(PlayerInfo[playerid][pTable] < 1)
			    {
					PlayerInfo[playerid][pTable] = 1;
	    			GivePlayerCash(playerid, -50000);
					Dyuze(playerid, "~y~Casino: ~w~Thank for buying poker table");
					SendClientMessage(playerid, COLOR_GREY, "SERVER: You have successfully acquired a poker table for 7 days.");
				}
				else SendClientMessage(playerid, COLOR_GREY, "You already have a poker table.");
			}
		}
	}
    ...
    #if defined PS_OnDialogResponse
        return PS_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
    #else
        return 1;
    #endif
}
...
#endif
#define OnDialogResponse PS_OnDialogResponse
#if defined PS_OnDialogResponse
    forward PS_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]);
#endif
...
public OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid)
{
	...
	#if defined PS_OnPlayerClickPlayerTextDraw
        return PS_OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid);
    #else
        return 1;
    #endif
}
...
#define OnPlayerClickPlayerTextDraw PS_OnPlayerClickPlayerTextDraw
#if defined PS_OnPlayerClickPlayerTextDraw
    forward PS_OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid);
#endif
...
PlacePokerTable(tableid, skipmisc, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz, virtualworld, interior)
{
	...
	// Create Table
	PokerTable[tableid][pkrObjectID] = CreateDynamicObject(OBJ_PS_TABLE, x, y, z, rx, ry, rz, virtualworld, interior, -1, DRAWDISTANCE_PS_TABLE);
	...
	new szString[64];
	format(szString, sizeof(szString), "[%d]\nPoker Table\nType '/jointable' to join the game!", tableid);
	PokerTable[tableid][pkrText3DID] = Create3DTextLabel(szString, COLOR_YELLOW, x, y, z+1.3, DRAWDISTANCE_PS_MISC, virtualworld, 0);
	...
	if(PokerTable[tableid][pkrPlayers] < PokerTable[tableid][pkrLimit])
	{
		...
		for(new s; s < 6; s++) {
			if(PokerTable[tableid][pkrSlot][s] == -1) {
				...
				ShowPokerGUI(playerid, GUI_PS_TABLE);
...
LeavePokerTable(playerid)
{
	...
	if(PokerTable[tableid][pkrPlayers] == 0) {
		...
		new tmpString[64];
		format(tmpString, sizeof(tmpString), "[%d]\nPoker Table\nType '/jointable' to join the game!", tableid);
...
CMD:buytable(playerid, params[])
{
	...
	if(PlayerInfo[playerid][pHours] < 2)
	{
	    return SendClientMessage(playerid, COLOR_SYNTAX, "You cannot have this loyal badge as you didn't play atleast 2 hours yet.");
	}
	ShowPlayerDialog(playerid, DIALOG_BUYPOKER, DIALOG_STYLE_LIST, "Casino", "7 days Poker Table ($50000)", "Purchase", "Cancel");
	return 1;
}
...
CMD:placetable(playerid, params[])
{
	...
	if(PlayerInfo[playerid][pTable] == 1 || PlayerInfo[playerid][pAdmin] >= 4)
	{
		...
		new string[128];
		format(string, sizeof(string), "SERVER: %s has placed a poker table!", GetPlayerNameEx(playerid));
	...
	{
		SendClientMessageEx(playerid, COLOR_GREY, "You don't have a poker table!");
	}
	return 1;
}

CMD:pokerhelp(playerid, params[])
{
    SendClientMessage(playerid, COLOR_GREEN,"_______________________________________");
    SendClientMessage(playerid, COLOR_YELLOW,"Poker Help:{FFFFFF} - type a command for more infomation.");
    SendClientMessage(playerid, COLOR_YELLOW,"Poker Help:{FFFFFF} /jointable /leavetable /placetable /destroytable");
	return 1;
}
...
CreatePokerGUI(playerid)
{
    ...
    PlayerPokerUI[playerid][41] = CreatePlayerTextDraw(playerid, 312.000000, 108.000000, "Leave");
    PlayerTextDrawFont(playerid, PlayerPokerUI[playerid][41], 2);
    PlayerTextDrawLetterSize(playerid, PlayerPokerUI[playerid][41], 0.183332, 1.500000);
    PlayerTextDrawTextSize(playerid, PlayerPokerUI[playerid][41], 337.000000, 18.500000);
    PlayerTextDrawSetOutline(playerid, PlayerPokerUI[playerid][41], 1);
    PlayerTextDrawSetShadow(playerid, PlayerPokerUI[playerid][41], 0);
    PlayerTextDrawAlignment(playerid, PlayerPokerUI[playerid][41], 1);
    ...
    PlayerTextDrawBoxColor(playerid, PlayerPokerUI[playerid][41], 74);
    ...
    PlayerPokerUI[playerid][42] = CreatePlayerTextDraw(playerid, 297.000000, 82.000000, "Poker Game");
    PlayerTextDrawFont(playerid, PlayerPokerUI[playerid][42], 2);
    PlayerTextDrawLetterSize(playerid, PlayerPokerUI[playerid][42], 0.204165, 1.500000);
    PlayerTextDrawTextSize(playerid, PlayerPokerUI[playerid][42], 351.500000, 18.500000);
    ...
    PlayerTextDrawAlignment(playerid, PlayerPokerUI[playerid][42], 1);
    ...
    PlayerTextDrawBoxColor(playerid, PlayerPokerUI[playerid][42], 74);
    PlayerTextDrawUseBox(playerid, PlayerPokerUI[playerid][42], 1);
    PlayerTextDrawSetProportional(playerid, PlayerPokerUI[playerid][42], 1);
    PlayerTextDrawSetSelectable(playerid, PlayerPokerUI[playerid][42], 0);

	PlayerPokerUI[playerid][43] = CreatePlayerTextDraw(playerid, 318.000000, 245.000000, "Texas Holdem Poker");
	PlayerTextDrawAlignment(playerid, PlayerPokerUI[playerid][43], 2);
	PlayerTextDrawBackgroundColor(playerid, PlayerPokerUI[playerid][43], -1);
	PlayerTextDrawFont(playerid, PlayerPokerUI[playerid][43], 2);
	PlayerTextDrawLetterSize(playerid, PlayerPokerUI[playerid][43], 0.199999, 1.200000);
	PlayerTextDrawColor(playerid, PlayerPokerUI[playerid][43], -1);
	...
	for(new i = 0; i < 6; i++) {
		...
		if(playerid != -1) {
			...
			if(PokerTable[tableid][pkrDelay] > 0 && PokerTable[tableid][pkrActive] == 3) {
				...
				if(playerid != -1) PlayerTextDrawSetString(playerid, PlayerPokerUI[playerid][43], tmpString);
			} else if(PokerTable[tableid][pkrActive] == 2) {
				...
				if(playerid != -1) PlayerTextDrawSetString(playerid, PlayerPokerUI[playerid][43], "Texas Holdem Poker");
			}
			// Community Cards
